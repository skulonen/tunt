    ...
    <calcite-shell>
      ...
      <calcite-navigation slot="header" style="--calcite-color-focus: transparent">
        <calcite-menu slot="content-start">
          ...
          >
            <calcite-date-picker
              ...
              lang="fi"
              valueAsDate=${date}
              ref=${el => el?.addEventListener('calciteDatePickerChange', event => {
        ...
        <calcite-menu slot="content-end">
          <calcite-menu-item
            active=${tab == 'home'}
            ref=${el => el?.addEventListener('calciteMenuItemSelect', () => setTab('home'))}
            icon-start="home"
            text="Home"
          />
          ${directory && html`
            <calcite-menu-item
              active=${tab == 'day'}
              ref=${el => el?.addEventListener('calciteMenuItemSelect', () => setTab('day'))}
              icon-start="clock"
              text="Day"
            />
            <calcite-menu-item
              active=${tab == 'summary'}
              ref=${el => el?.addEventListener('calciteMenuItemSelect', () => setTab('summary'))}
              icon-start="filter"
              text="Summary"
            />
          `}
